    ...
    <>
      ...
      {view === 'profile' ? (
        <label
          ...
          className={classNames(
            classnames,
            'relative h-[135px] w-[135px] cursor-pointer rounded-full border border-dashed border-slate-200 bg-white focus:outline-secondary'
          )}
          draggable={true}
          onKeyDown={(e) => {
            if (isUploading) return;
        ...
        >
          <div className='flex h-full w-full flex-col items-center justify-center space-y-1 rounded-full bg-gray-50 text-sm transition-all duration-75 hover:opacity-80'>
            {isUploading ? (
              <div role='status'>
                <svg
                  aria-hidden='true'
